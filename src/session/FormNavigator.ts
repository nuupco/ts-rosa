...
export class FormNavigator {
  ...
  stepToPreviousEvent(): FormEntryEvent {
    ...
    while (isAt(prev)) {
      if (!this.isStopRelevant(prev)) {
        prev = this.decrementIndex(prev);
        continue;
      }
      // Skip the exhausted count-controlled repeat's "next slot" junction —
      // symmetric with stepToNextEvent's skip. decrementHelper/
      // setRepeatNextMultiplicity always produces this junction on the way
      // back (mirroring incrementHelper, which produces it going forward
      // too), but stepToNextEvent never presents it as a stop once the
      // repeat is exhausted, so stepping backward must not stop there either.
      if (this.isExhaustedCountRepeat(prev)) {
        prev = this.decrementIndex(prev);
        continue;
      }
      break;
  ...
  private setRepeatNextMultiplicity(levels: MutableLevel[]): boolean {
    ...
    levels[levels.length - 1]!.multiplicity = count;
    return true;
  }
}
