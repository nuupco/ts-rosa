...
| @getodk/xforms-engine + `packages/scenario` | **Reference only (design + behavior cross-check)** | Not compiled into ts-rosa; no `@getodk/xforms-engine` import anywhere in the source tree. `packages/scenario` is the design inspiration for our own `Scenario` harness (Phase 0). ~11 individual `it()` test scenarios across 6 equivalence files (repeat.test.ts, indexed-repeat.test.ts, relevant.test.ts, dag/calculate.test.ts, constraint.test.ts, secondary-instances/calculate-predicates.test.ts) have been adapted from `reference/web-forms` — re-implemented in ts-rosa's own Scenario DSL and attributed inline with `// Source: web-forms/...` comments. No web-forms code is imported or executed. |
