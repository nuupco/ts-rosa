...
### Fixed
- `FormEvaluator.validate()` only checked `required`, rank, and `constraint` against a repeat's first (default) instance: it resolved each generic bind nodeset with `resolveReference`, which always resolves an unbound-index ref to instance 0. Violations in any instance beyond the first went undetected. Now uses `resolveAll` to expand repeat wildcards to every instance and derives a concrete per-instance ref for each check.
